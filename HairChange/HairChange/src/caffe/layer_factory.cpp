...
// Get DummyData layer according to engine.
template <typename Dtype>
shared_ptr<Layer<Dtype> > GetDummyDataLayer(const LayerParameter& param) {
    return shared_ptr<Layer<Dtype> >(new DummyDataLayer<Dtype>(param));
}

REGISTER_LAYER_CREATOR(DummyData, GetDummyDataLayer);

// Get ELU layer according to engine.
template <typename Dtype>
...
// Get Embed layer according to engine.
template <typename Dtype>
shared_ptr<Layer<Dtype> > GetEmbedLayer(const LayerParameter& param) {
    return shared_ptr<Layer<Dtype> >(new EmbedLayer<Dtype>(param));
}

REGISTER_LAYER_CREATOR(Embed, GetEmbedLayer);
...
// Get DenseImageData layer according to engine.
template <typename Dtype>
shared_ptr<Layer<Dtype> > GetDenseImageDataLayer(
			const LayerParameter& param) {
    return shared_ptr<Layer<Dtype> >(new DenseImageDataLayer<Dtype>(param));
}

REGISTER_LAYER_CREATOR(DenseImageData, GetDenseImageDataLayer);

 
// Get SoftmaxWithLoss layer according to engine.
template <typename Dtype>
shared_ptr<Layer<Dtype> > GetSoftmaxWithLossLayer(
			const LayerParameter& param) {
...
// Get Accuracy layer according to engine.
template <typename Dtype>
shared_ptr<Layer<Dtype> > GetAccuracyLayer(
			const LayerParameter& param) {
...
//Get ArgMax layer according to engine.
template <typename Dtype>
shared_ptr<Layer<Dtype> > GetArgMaxLayer(
	const LayerParameter& param) {
...
//Get SpatialDropout layer according to engine.
template <typename Dtype>
shared_ptr<Layer<Dtype> > GetSpatialDropoutLayer(
	const LayerParameter& param) {
	return shared_ptr<Layer<Dtype> >(new SpatialDropoutLayer<Dtype>(param));
}

REGISTER_LAYER_CREATOR(SpatialDropout, GetSpatialDropoutLayer);
...
shared_ptr<Layer<Dtype> > GetPythonLayer(const LayerParameter& param) {
    ...
    try {
        ...
        bp::object layer = module.attr(param.python_param().layer().c_str())(param);  //(boost::ref(param));
